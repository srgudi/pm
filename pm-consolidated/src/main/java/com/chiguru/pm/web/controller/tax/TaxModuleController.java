...
public class TaxModuleController {
    ...
    @ModelAttribute("clientPracticeInfo")
...
//                rule.setLastUpdateUserId("system");
     ...
     */
    @RequestMapping(value = {"/tax/taxModule.do","/mobiletax/mobileTaxModule.do"}, method = RequestMethod.GET)
    public String getTaxModule(HttpServletRequest request,@ModelAttribute("clientPracticeInfo")ClientPracticeInfo info, Long firmPracticeId, Long clientId,
        ModelMap model) {
        ...
        if(info.getFilingDueDateRules().isEmpty()){
        	List<FilingDueDateRule> filingDueDateList = new ArrayList();
        	filingDueDateList.add(new FilingDueDateRule());
        	info.setFilingDueDateRules(filingDueDateList);
        }
        if(info.getInstallmentDueDateRules().isEmpty()){
        	List<InstallmentDueDateRule> installmentDueDateList = new ArrayList();
        	installmentDueDateList.add(new InstallmentDueDateRule());
        	info.setInstallmentDueDateRules(installmentDueDateList);
        }
        model.addAttribute("clientPracticeInfo", info);
        Long firmId = PMUtil.getFirmIdFromSession(request);
        ...
        if(info != null){
        	...
        	if(client != null)
        	{
              clientName = client.getName();
        		...
        		client = crmManager.getPartyById(clientId);
        	}
        if(clientName != null){
        ...
        if(request.getRequestURI().contains("/mobiletax/mobileTaxModule.do")){
        	return "mobileTaxModule.tilesView";
        }else{
        return "tax/taxModule";
        }
    }

    @RequestMapping(value = "/tax/taxModuleCol.do", method = RequestMethod.GET)
    public String getTaxModuleCol(HttpServletRequest request,
    		@ModelAttribute("clientPracticeInfo")ClientPracticeInfo practiceInfo, Long firmPracticeId, Long clientId,
        ModelMap model) {
        logger.info("Tax Module!");
        HttpSession session1 = request.getSession();
		if (clientId == null) {
        ...
        //return "redirect:/Chiguru/taxModule.do?id="+firmPracticeId+"&firmId=" + firmId+"&clientId="+clientId;
        return "tax/taxModuleCol";
    }
    /**
     * Simply selects the welcome view to render by returning void and relying
     * on the default request-to-view-translator.
     */
    @RequestMapping(value = {"/tax/taxModule.do","/mobiletax/mobileTaxModule.do"}, method = RequestMethod.POST)
    public  ResponseEntity<String> saveTaxModule(HttpServletRequest request,
    	ClientPracticeInfo info, Long firmPracticeId, Long clientId,
        ModelMap model, List[] delteListArray) {
        logger.info("Tax Module!");
        HttpHeaders responseHeaders = new HttpHeaders();
        responseHeaders.setContentType(MediaType.TEXT_HTML);
        Long firmId = PMUtil.getFirmIdFromSession(request);
        Long firmPracticeType = info.getFirmPracticeType().getId();
        FirmPracticeType type = taxModuleManager.getFirmPracticeTypeById(firmPracticeType);
        info.setFirmPracticeType(type);
        if (!request.getRequestURI().contains("/appadmin/adminFirmModule.do") &&
                (firmId == null)) {
            throw new RuntimeException("firmId can not be empty");
        ...
        populateRulesProperly(info, delteListArray);
        taxModuleManager.saveTaxPracticeInfo(info, delteListArray);
    ...
    private List<Party> getStaffData(Long partyId) {
        return crmManager.getRelatedPartiesById(partyId,
            RelationshipType.RELATIONSHIP_TYPE_CLIENT_STAFF.getId());
    }
