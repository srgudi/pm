...
public class TextEncryptUtil {
    private static final StandardPBEStringEncryptor STANDARD_PBE_STRING_ENCRYPTOR;
    private static final String password = "BoomShak*6laka@#12!3%%fsfhjsdgadityafdhfbsdmfsjfbhf$$###$@$@$@$@$%%#%#vvdfsfsf";

    private TextEncryptUtil() {
    }

    static {
        // Register bouncy castle provider
        Security.addProvider(new BouncyCastleProvider());
        STANDARD_PBE_STRING_ENCRYPTOR = new StandardPBEStringEncryptor();
        STANDARD_PBE_STRING_ENCRYPTOR.setPassword(password);
        // Set out to hex encoding, base64 default does not work
        STANDARD_PBE_STRING_ENCRYPTOR.setStringOutputType("hexadecimal");
        // choose an algorithm
        STANDARD_PBE_STRING_ENCRYPTOR.setAlgorithm("PBEWITHSHA256AND256BITAES-CBC-BC");
    }

    public static String encrypt(String val) {
        return STANDARD_PBE_STRING_ENCRYPTOR.encrypt(val.toString());
    }

    public static Long decrypt(String val) {
        return Long.valueOf(STANDARD_PBE_STRING_ENCRYPTOR.decrypt(val));
    }
    
    public static String encryptForLong(Long val) {
    	String encrypted = null;
    	try {
    		String valInStr = val.toString();
    		encrypted = STANDARD_PBE_STRING_ENCRYPTOR.encrypt(valInStr);
        } catch (EncryptionOperationNotPossibleException exception) {
            throw new RuntimeException("Encryption not possible for: " + val);
        }
    	return encrypted;
    }

    public static Long decryptToLong(String val) throws RuntimeException {
        Long asLong = null;
        try {
            String decrypted = STANDARD_PBE_STRING_ENCRYPTOR.decrypt(val);
            asLong = Long.valueOf(decrypted);
        } catch (EncryptionOperationNotPossibleException exception) {
            throw new RuntimeException("Encryption not possible for: " + val);
        }
        return asLong;
     ...
     * @return
     */
    public static List<KeyValueDropDown> encryptDropDown(List<KeyValueDropDown> dropDownList) {
        if (dropDownList == null) {
            return null;
        }
        for (KeyValueDropDown dropDown : dropDownList) {
            String entityKey = encrypt(dropDown.getId().toString());
            dropDown.setEntityKey(entityKey);
            // Blank out primary key, do not show
            dropDown.setId(null);
        }
        return dropDownList;
    }

    public static String getMatchingEntityKey(Long id, List<KeyValueDropDown> keyValueDropDowns) {
        String key = null;
        for (KeyValueDropDown keyValueDropDown : keyValueDropDowns) {
            String entityKey = keyValueDropDown.getEntityKey();
            Long decryptId = Long.valueOf(TextEncryptUtil.decrypt(entityKey));
            if (decryptId.equals(id)) {
                key = entityKey;
                break;
            }
        }
    ...
    public static List<Long> decryptItemsAsList(String[] items) {
        List<Long> idList = null;
        if (items != null && items.length > 0) {
            idList = new ArrayList<Long>();
            for (String item : items) {
                Long id = decryptToLong(item);
                idList.add(id);
            }
        }
        return idList;
...
//--- Documentation purposes only, please do not mess around
/*
KeyGenerator:
            HMAC-MD2
            GOST
