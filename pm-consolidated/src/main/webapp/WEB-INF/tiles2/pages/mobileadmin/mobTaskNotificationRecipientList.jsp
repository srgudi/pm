...
<div id="selectDelRow" class="modal" style="display: none; margin-top:10">
	...
	<table  border="0" class="table table-striped table-hover table-bordered mb20" id="getrecipientwmob">
	<colgroup>
					<col style="width: 20%;">
					<col style="width: 20%;">
					<col style="width: 20%;">
...
$('#getrecipientwmob').dataTable( {
	/* "bJQueryUI": true, */
	"bJQueryMobileUI" : true,
	"sPaginationType" : "bootstrap",
	"bLengthChange": true,
	"sAjaxSource": '/pm-new/appadmin/taskNotificationRecipientList.do',
	"bFilter": true,
    "bAutoWidth": false,
    "sInfoFiltered": " ",
...
$('#deletedRecord').click(function() {
	...
	if ($(this).data('action') == 'deletedRecord') {
		...
		URL = '/pm-new/mobileadmin/saveTaskNotificationRecipient.do?id=' + selectedItems;
	}
	...
	$.ajax({
		...
		success : function(data) {
			if (data) {
				$('#getrecipientwmob').dataTable().fnDraw();
...
function editRecepient(partyId) {
	//alert("hi");
	window.location = 'mobileAdminTaskNotificationRecepient.do?id=' + partyId;
	//showTaxform();
	return false;
}
</script>	
