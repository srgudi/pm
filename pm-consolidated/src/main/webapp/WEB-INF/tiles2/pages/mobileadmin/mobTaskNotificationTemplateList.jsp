...
<div id="selectDelRow" class="modal" style="display: none; margin-top:10">
	...
	<table  border="0" class="table table-striped table-hover table-bordered mb20" id="gettemplatewmob">
	<colgroup>
					<col style="width: 20%;">
					<col style="width: 20%;">
					<col style="width: 20%;">
...
$('#gettemplatewmob').dataTable( {
	/* "bJQueryUI": true, */
	"bJQueryMobileUI" : true,
	"sPaginationType" : "bootstrap",
	"bLengthChange": true,
	"sAjaxSource": '/pm-new/appadmin/taskNotificationTemplateList.do',
	"bFilter": true,
    "bAutoWidth": false,
    "sInfoFiltered": " ",
...
$('#deletedRecord').click(function() {
	...
	if ($(this).data('action') == 'deletedRecord') {
		...
		URL = '/pm-new/mobileadmin/saveTaskNotificationTemplate.do?id=' + selectedItems;
	}
	...
	$.ajax({
		...
		success : function(data) {
			if (data) {
				$('#gettemplatewmob').dataTable().fnDraw();
...
function editTemplate(partyId) {
	//alert("hi");
	window.location = 'mobileadminTaskNotificationTemplate.do?id=' + partyId;
	//showTaxform();
	return false;
}
</script>	
