...
<c:url value="/mobileadmin/mobileAdminFirmEmployee.do?sessionfirmId=${sessionfirmId}" var="mobileemployeeUrl"/>  

<a href="<c:out value='${mobileemployeeUrl}'/>" style="text-decoration:none;">
...
<div id="selectDelRow" class="modal" style="display: none; margin-top:10">
	...
	<table  border="0" class="table table-striped table-hover table-bordered mb20" id="getfirmempListmob">
	<colgroup>
					<col style="width: 10%;">
					<col style="width: 20%;">
					<col style="width: 20%;">
...
$('#getfirmempListmob').dataTable( {
	/* "bJQueryUI": true, */
	"bJQueryMobileUI" : true,
	"sPaginationType" : "bootstrap",
	"bLengthChange": true,
	"sAjaxSource": '/pm-new/appadmin/mainEmployeeList.do?newfirmId='+firmId,
	"bFilter": true,
    "bAutoWidth": false,
    "sInfoFiltered": " ",
...
$('#deletedRecord').click(function() {
	...
	if ($(this).data('action') == 'deletedRecord') {
		...
		URL = '/pm-new/appadmin/saveClientAjaxList.do?id=' + selectedItems;
	}
	...
	$.ajax({
		...
		success : function(data) {
			if (data) {
				$('#getfirmempListmob').dataTable().fnDraw();
...
function EmpEdit(partyId) {
	window.location = '/pm-new/mobileadmin/mobileAdminFirmEmployee.do?id=' + partyId;
	return false;
}
</script>	
</div>