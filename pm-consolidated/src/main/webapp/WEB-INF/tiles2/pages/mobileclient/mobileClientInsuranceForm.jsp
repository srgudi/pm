...
<style type="text/css">
    .insurancePartyAccount fieldset p.error label {
        color: red;
    }
    ...
    form.insurancePartyAccount {
        width: 30em;
    }

    form.insurancePartyAccountlabel.error {
        display: block;
        margin-left: 1em;
        width: auto;
    }
    ...
    $().ready(function() {
    	var insuId = $("#id").val();
    	 ...
    	 $("#saveins").click(function() {
    		 if($("#insurancePartyAccount").validate({
    	            ...
    	            rules: {
    	                ...
    	                'address.addressLine2': {required:true,addressValidation:true,minlength: 0,maxlength: 100},
    	                'address.city': {required:true,alphabetsOnly:true,minlength: 0,maxlength: 50},
    	                'address.countryCode': {},
    	                'address.stateCode': {},
             	...
             	$.ajax({
			        ...
			        url: 'mobileClientInsurance.do',
			        success: function(clientId) {
			        	if(clientId != -1){
				        	$.jGrowl("Record saved successfully");	
	                		 if(status==true){
	                			var clientId = ${clientId};
	                			 window.location = 'mobilecInsurance.do?clientId='+clientId+'&id='+insuId;
	                		}else{
	                			var clientId = ${clientId};
	                			document.getElementById("insurancePartyAccount").reset();
	                			$('#getAllIns').dataTable().fnReloadAjax('mobileAccountAjaxList.do?clientId=' + clientId + '&accountCategory=701');
	                		}  
			        	}
			        	 else{
...
<c:url value="/mobileclient/mobilecInsurance.do" var="mobileInsuranceModuleUrl">  
...
<form:form name="partyAccount" modelAttribute="insurancePartyAccount" method="POST"
...
<b><strong>${inValidClientId}</strong></b>
           ...
           <div data-role="fieldcontain" >
             <c:forEach
                    items="${insurancePartyAccount.defaultPointOfContact.emailList}"
           ...
           <form:label class ="ui-input-text" path="defaultPointOfContact.emailList">Email Address</form:label> 
                ...
                <div data-role="fieldcontain" >
             <c:forEach
                    items="${insurancePartyAccount.defaultPointOfContact.faxList}"
