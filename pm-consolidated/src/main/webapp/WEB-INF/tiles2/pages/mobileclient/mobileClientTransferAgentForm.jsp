...
<style type="text/css">
    .agentPartyAccount fieldset p.error label {
        color: red;
    }
    ...
    form.agentPartyAccountlabel.error {
        display: block;
        margin-left: 1em;
        width: auto;
    }
    	 ...
    	 $("#saveTran").click(function() {
    		 if($("#agentPartyAccount").validate({
             	...
             	$.ajax({
			        ...
			        url: 'mobileClientTAgent.do',
			        success: function(clientId) {
			        	if(clientId != -1){
				        	$.jGrowl("Record saved successfully");	
	                		 if(status==true){
	                			var clientId = ${clientId};
	                			 window.location = 'mobilecTAgent.do?clientId='+clientId+'&id='+agentId;
	                		}else{
	                			var clientId = ${clientId};
	                			document.getElementById("agentPartyAccount").reset();
	                			$('#getAllTransAg').dataTable().fnReloadAjax('mobileAccountAjaxList.do?clientId=' + clientId + '&accountCategory=702');
	                		}  
...
<form:form name="partyAccount" modelAttribute="agentPartyAccount" method="POST"
...
<!-- BEGIN ROW 1 -->
           ...
           <form:label class ="ui-input-text" path="defaultPointOfContact.emailList">Email Address</form:label> 
                ...
                <div data-role="fieldcontain" >
             <c:forEach
                    items="${agentPartyAccount.defaultPointOfContact.faxList}"
          ...
          <div data-role="fieldcontain" >
                    <c:forEach
                    items="${agentPartyAccount.defaultPointOfContact.webUrlList}"
                    var="contact" varStatus="cStatus">
            <form:label class ="ui-input-text" path="defaultPointOfContact.webUrlList">Web&nbsp;&nbsp;</form:label>      
             ...
             <form:label class ="ui-input-text" path="defaultPointOfContact.phoneList">Phone No.</form:label>
            			...
            			<c:forEach items="${agentPartyAccount.defaultPointOfContact.phoneList}" var="contact" varStatus="cStatus">
			                <table width="100%">
			                    <tr class="phonerow">
