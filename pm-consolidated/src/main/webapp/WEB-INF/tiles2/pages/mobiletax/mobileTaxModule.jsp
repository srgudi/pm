...
<c:url value="/mobiletax/mobileTaxList.do" var="mobiletaxModuleUrl">
...
<form:form modelAttribute="clientPracticeInfo" method="POST"
	action="mobileTaxModule.do">
	<input type="hidden" name="clientId" id="clientId"
		value="${param.clientId }" />
	<input type="hidden" name="firmPracticeId" id="firmPracticeId"
		value="${param.firmPracticeId}" />
	<input type="hidden" name="baseURL" id="baseURL"
		value="mobileTaxModule.do" />
	...
	<div data-role="fieldcontain">
		...
		<form:label path="jurisdiction">Jurisdiction</form:label>
			<form:select path="jurisdiction">
				...
				<form:options items="${jurisdictionList}" itemValue="id"
					itemLabel="name" />
			</form:select>
		
	</div>
	<div data-role="fieldcontain">
		<form:label class="ui-input-text" path="filingFrequency">Filing Frequency</form:label>
		<form:select path="filingFrequency" id="filingFreqField" tabindex="1"
			cssClass="input_full" data-mini="true">
			<form:option value="-1" label="Select" />
			<form:options items="${filingFrequency}" itemValue="id"
				itemLabel="shortName" />
		</form:select>
	</div>
	<div class="filing_month_date">
		<div data-role="fieldcontain">
			<c:forEach items="${clientPracticeInfo.filingDueDateRules}"
				var="dateRule" varStatus="cStatus">
				<form:label path="filingDueDateRules[${cStatus.index}].dueMonth">
					<div class="filing_due_month_label">Due Month</div>
				</form:label>

				<form:select path="filingDueDateRules[${cStatus.index}].dueMonth"
					title="filingMonthField" name="filingMonthField" tabindex="3"
					cssClass="input_full" data-mini="true">
					<form:option value="-1" label="Select" />
				...
				<form:label path="filingDueDateRules[${cStatus.index}].dueDay">
					<div class="filing_due_day_label">Due Date</div>
				</form:label>

				<form:select path="filingDueDateRules[${cStatus.index}].dueDay"
					title="filingDayField" name="filingDayField" tabindex="4"
					cssClass="input_full" data-mini="true">
					<form:option value="-1" label="Select" />
				...
				<form:hidden path="filingDueDateRules[${cStatus.index}].id"
					class="filingDateId" />
			</c:forEach>
		</div>
	</div>
	...
	<div data-role="fieldcontain">
		<form:label class="ui-input-text" path="filingDateReminder">Filing Reminders</form:label>
		<form:select path="filingDateReminder" id="filingDateReminderField"
			tabindex="1" cssClass="input_full" data-mini="true">
	...
	<div data-role="fieldcontain">
		<div class="reminder_days_row"
			style="<c:out value='${filing_reminder_display}'/>">
			<form:label path="filingDateReminder">Reminders Days</form:label>
			<fieldset data-role="controlgroup" data-mini="true"
				data-type="horizontal">
				<c:forEach items="${filingReminderDays}" var="reminderCode"
					varStatus="cStatus">
					<c:set var="checked" value="" />
					<c:set var="id" value="-1" />
					<c:forEach items="${clientPracticeInfo.filingReminderRules}"
						var="reminderRule" varStatus="ruleIndex">
					...
					<input type="checkbox"
						id="filingReminderRules${cStatus.index}.reminderDayCode"
						name="filingReminderRules[${cStatus.index}].reminderDayCode"
						value="${reminderCode.id }" class="custom"
						<c:out
	                                    value="${checked}"/> />
					<label style="width: 60px;"
						for="filingReminderRules${cStatus.index}.reminderDayCode"><c:out
							value="${reminderCode.shortName}" /></label>
					<input type="hidden" id="filingReminderRules${cStatus.index}"
						class="custom" name="filingReminderRules[${cStatus.index}]"
						value="${id }" />
	...
	<div data-role="fieldcontain">
		<form:label class="ui-input-text" path="instalmentFrequency">Installment Frequency</form:label>

		<form:select path="instalmentFrequency" id="instalmentFreqField"
			tabindex="1" cssClass="input_full" data-mini="true">
			<form:option value="-1" label="Select" />
			<form:options items="${instalmentFrequency}" itemValue="id"
				itemLabel="shortName" />
		</form:select>
	</div>
...
<div class="instalment_month_date">
	<div data-role="fieldcontain">
			<c:forEach items="${clientPracticeInfo.installmentDueDateRules}"
				var="dateRule" varStatus="cStatus">
				<form:label
					path="installmentDueDateRules[${cStatus.index}].dueMonth">
					<div class="installment_due_month_label">Due Month</div>
				</form:label>

				<form:select
					path="installmentDueDateRules[${cStatus.index}].dueMonth"
					title="installmentMonthField" tabindex="3" cssClass="input_full"
					data-mini="true">
					<form:option value="-1" label="Select" />
					<form:options items="${monthList}" itemValue="value"
						itemLabel="key" />
				</form:select>
				<form:label path="installmentDueDateRules[${cStatus.index}].dueDay">
					<div class="installment_due_day_label">Due Date</div>
				</form:label>

				<form:select path="installmentDueDateRules[${cStatus.index}].dueDay"
					title="installmentDayField" tabindex="3" cssClass="input_full"
					data-mini="true">
					<form:option value="-1" label="Select" />
					<c:forEach var="i" begin="1" end="31" step="1">
						<form:option value="${i}" label="${i}" />
				...
				<form:hidden path="installmentDueDateRules[${cStatus.index}].id"
					class="installmentDateId" />
			</c:forEach>
		</div>
	</div>

	<div data-role="fieldcontain">
		<form:label class="ui-input-text" path="instalmentReminder">Installment Reminders</form:label>

		<form:select path="instalmentReminder"
			id="instalmentDateReminderField" tabindex="1" cssClass="input_full"
			data-mini="true">
			<form:options items="${yesNoList}" itemValue="value" itemLabel="key" />
		</form:select>
	...
	<div data-role="fieldcontain">
		<div class="installment_days_row"
			style="<c:out value='${installment_reminder_display}'/>">
			<form:label path="filingDateReminder">Reminders Days</form:label>
			<fieldset data-role="controlgroup" data-mini="true"
				data-type="horizontal">
				<c:forEach items="${instalmentReminderDays}" var="reminderCode"
					varStatus="cStatus">
					<c:set var="checked" value="" />
					<c:set var="id" value="-1" />
					<c:forEach items="${clientPracticeInfo.installmentReminderRules}"
						var="reminderRule" varStatus="ruleIndex">
					...
					<input type="checkbox"
						id="installmentReminderRules${cStatus.index}.reminderDayCode"
						name="installmentReminderRules[${cStatus.index}].reminderDayCode"
						value="${reminderCode.id }" class="custom"
						<c:out
	                                    value="${checked}"/> />
					<label style="width: 60px;"
						for="installmentReminderRules${cStatus.index}.reminderDayCode"><c:out
							value="${reminderCode.shortName}" /></label>

					<input type="hidden" id="installmentReminderRules${cStatus.index}"
						class="custom" name="installmentReminderRules[${cStatus.index}]"
						value="${id }" />
	...
	<div data-role="fieldcontain">
		...
		<form:select path="clientStaffInCharge" cssClass="input_full"
			tabindex="1" data-mini="true">
			<form:option value="-1" label="Select" />
			<form:options items="${clientStaffList}" itemValue="id"
				itemLabel="name" />
		</form:select>
	</div>
	...
	<div data-role="fieldcontain">
		...
		<form:textarea path="preparerNotes" rows="3" cssClass="input_full"
			tabindex="16" />
	...
	<a href="<c:out value = '${mobiletaxModuleUrl}' />"
...
<script type="text/javascript" charset="utf-8">
	...
	$(document)
			.ready(
					function() {
						...
						$("#filingDueDateRules0\\.dueMonth")
								.change(function() {
											$.get("<c:url value='/mobiletax/getDays.do'/>",
															{
																monthId : $(this).val()
															},
															function(data) {
						...
						$("#filingFreqField")
								.change(
										function() {
											var label = $(
													'#filingFreqField option:selected')
													.text();
											var freq = $("#filingFreqField")
													.val();
											...
											if (filingFields < count) {
												addFilingField(count,
														filingFields);
											} else if (filingFields > count) {
												removeFilingFrequencyField(filingFields
														- count);
											}
										});
						...
						$("#instalmentFreqField")
								.change(
										function() {
											var label = $(
													'#instalmentFreqField option:selected')
													.text();
											var freq = $("#instalmentFreqField")
													.val();
											...
											if (filingFields < count) {
												addInstallmentField(count,
														filingFields);
											} else if (filingFields > count) {
												removeInstallmentFrequencyField(filingFields
														- count);
											}
										});
						...
						$("#instalmentDateReminderField").change(
								function() {
									var reminder = $(
											"#instalmentDateReminderField")
											.val();
									if (reminder == 'true') {
						...
						$("#installmentDueDateRules0\\.dueMonth")
						.change(function() {
									$.get("<c:url value='/mobiletax/getDays.do'/>",
													{
														monthId : $(this).val()
													...
													function(data) {
														populateStateDropdown($("#installmentDueDateRules0\\.dueDay"),data);
	...
	function disableInstFrqReminderField() {
		$("#installmentReminderRules5\\.reminderDayCode")
				.attr('disabled', true);
		$("#installmentReminderRules6\\.reminderDayCode")
				.attr('disabled', true);
	}
	...
	function addFilingField(newCount, existingFields) {
		...
		for ( var i = existingFields; i < newCount; i = i + 1) {
			...
			$('.filing_month_date:last').find(
					"div[class='filing_due_month_label']").text(
					'Due Month ' + filingFieldCount);
			$('.filing_month_date:last').find(
					"div[class='filing_due_day_label']").text(
					'Due Date ' + filingFieldCount);

			$('.filing_month_date:last').find("select[title='filingDayField']")
					.attr({
						name : 'filingDueDateRules[' + (i) + '].dueDay',
						id : 'filingDueDateRules[' + (i) + '].dueDay'
					}).val("");

			$('.filing_month_date:last').find("input[class='filingDateId']")
					.attr({
	...
	function addInstallmentField(newCount, existingFields) {
		$('.instalment_month_date:first').find(
				"div[class='installment_due_month_label']").text('Due Month 1');
		$('.instalment_month_date:first').find(
				"div[class='installment_due_day_label']").text('Due Date 1');
		filingFieldCount = existingFields;
		for ( var i = existingFields; i < newCount; i = i + 1) {
			filingFieldCount = filingFieldCount + 1;

			$('.instalment_month_date:last').clone().insertAfter(
					".instalment_month_date:last").find(
					"select[title='installmentMonthField']").attr({
				name : 'installmentDueDateRules[' + (i) + '].dueMonth',
				id : 'installmentDueDateRules[' + (i) + '].dueMonth'
			}).val("");

			$('.instalment_month_date:last').find(
					"div[class='installment_due_month_label']").text(
					'Due Month ' + filingFieldCount);
			$('.instalment_month_date:last').find(
					"div[class='installment_due_day_label']").text(
					'Due Date ' + filingFieldCount);

			$('.instalment_month_date:last').find(
					"select[title='installmentDayField']").attr({
				name : 'installmentDueDateRules[' + (i) + '].dueDay',
				id : 'installmentDueDateRules[' + (i) + '].dueDay'
			}).val("");

			$('.instalment_month_date:last').find(
					"input[class='installmentDateId']").attr({
...
$(document).ready(function() {
	$().ready(function() {
						...
						$("#save").click(function() {
											//alert("c" + clientId + "f" +firmPracticeId);
											$.ajax({
														dataType : 'json',
														type : 'POST',
														data : $('#clientPracticeInfo')
																.serialize(),
														url : 'mobileTaxModule.do?clientId='+ clientId+ '&firmPracticeId='+ firmPracticeId,
														complete : function(clientId) {
															if (clientId != -1) {
																$.jGrowl("Record saved successfully");
																if (editStatus == true) {
																	var clientId = ${clientId};
																	window.location = 'mobileTaxModule.do?id='+ taxId;
																} else {
																	document.getElementById("clientPracticeInfo").reset();
																	$('#getTaxList').dataTable().fnReloadAjax('mobileTaxModuleList.do?clientId='+ clientId+ '&firmPracticeId='+ firmPracticeId);
																}
															} else {
						...
						var month11 = document
								.getElementById("filingDueDateRules0.dueMonth");
						var month1 = month11.value;
						var date11 = document
								.getElementById("filingDueDateRules0.dueDay");
	...
	function isDate(dtStr) {
		...
		if (strYear.length != 4 || year == 0 || year<minYear || year>maxYear) {
			$.jGrowl('Please enter a valid 4 digit year between ' + minYear
					+ ' and' + maxYear);
			return false;
		}
		if (dtStr.indexOf(separator, pos2 + 1) != -1
				|| isInteger(stripCharsInBag(dtStr, separator)) == false) {
