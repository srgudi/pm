...
//            contact.setCreateUserId("system");
            System.out.println("I AM HERE"+contact);
            logger.info("I AM HERE"+contact.getPhoneList().get(0).getContactDetail());
        }
...
//	            contact.setCreateUserId("system");
 ...
 //               System.out.println("I AM HERE");
    ...
    public List<PhoneCommunication> initializeModelForDeletes(
        PartySite partySite, HttpServletRequest request) {
    ...
    /*@ModelAttribute
    public List<PartySiteCommunication> initializeModelForDeletes (PartySite partySite, HttpServletRequest request){
            List<PartySiteCommunication> toBeDeletedList = new ArrayList();
            if (request.getMethod().equals("POST")){
                    List<PartySiteCommunication> commList = partySite.getPartySiteCommunications();
                    toBeDeletedList = removeDeletedContactEntries(toBeDeletedList, commList, request);
            }
            return toBeDeletedList;
    }*/

    /*private List<PartySiteCommunication> removeDeletedContactEntries(List<PartySiteCommunication> toBeDeletedList,
                    List<PartySiteCommunication> commListFromDB, HttpServletRequest request){
            Map paraMap = request.getParameterMap();
            Set keySet = paraMap.keySet();
            for(Iterator itComm =  commListFromDB.iterator(); itComm.hasNext();){
                    PartySiteCommunication comm = (PartySiteCommunication)itComm.next();
                    if (comm != null) {
                            boolean exists = false;
    
                            for (Iterator it = keySet.iterator(); it.hasNext();) {
                                    String key = (String) it.next();
                                    if (key.contains("partySiteCommunications")
                                                    && key.contains("id")) {
                                            String value = request.getParameter(key);
                                            if (comm.getId() != null
                                                            && comm.getId().toString().equals(value)) {
                                                    exists = true;
                                                    break;
                                            }
    ...
    @RequestMapping(value = "/client/clientLocation.do", method = RequestMethod.POST)
    public String saveClientLocation(HttpServletRequest request,
    @Valid PartySite partySite, SessionStatus status,
        List<PhoneCommunication> phonesToBeRemoved, ModelMap model ) {
        logger.info("Client Location Save!");
        List<PhoneCommunication> phonesToBeDeletedList = new ArrayList();
        //		Long firmId = Long.valueOf(request.getParameter("firmId"));
        Long firmId = PMUtil.getFirmIdFromSession(request);
        BindingResult result = (BindingResult) model.get("org.springframework.validation.BindingResult.partySite");
        if(result.hasErrors()) {
        	this.getClient(request, model, partySite);
        	return null;
        }
        ...
        //		return "redirect:/Chiguru/clientLocation.do?id="+partySite.getId()+"&clientId="+clientId+"&saveSuccess=true";

        //		String baseURL = request.getParameter("baseURL");
        //		String returnURL = baseURL + "?id="+partySite.getId()+"&saveSuccess=true";
        String returnURL = "/client/clientLocation.do?id=" + partySite.getId() +
            "&clientId=" + clientId + "&saveSuccess=true";

        return "redirect:" + returnURL;
    }

    @RequestMapping(value = "/mobileclient/mobileclientLocation.do", method = RequestMethod.POST)
    public String saveMobileClientLocation(HttpServletRequest request,
    @Valid PartySite partySite, SessionStatus status,
        List<PhoneCommunication> phonesToBeRemoved, ModelMap model ) {
        logger.info("Client Location Save!");
        List<PhoneCommunication> phonesToBeDeletedList = new ArrayList();
        ...
        //		String baseURL = request.getParameter("baseURL");
        //		String returnURL = baseURL + "?id="+partySite.getId()+"&saveSuccess=true";
        String returnURL = "/mobileclient/mobileclientLocation.do?id=" + partySite.getId() +
            "&clientId=" + clientId + "&saveSuccess=true";
...
//            communication.setLastUpdateUserId("system");
    ...
    public String manageBankBranch(PartySite partySite, BindingResult result,
        ...
        saveLocation(request, partySite, result, status, phonesToBeRemoved);
     ...
     * on the default request-to-view-translator.
     */
    @RequestMapping(value = "/client/clientLocation.do", method = RequestMethod.GET)
    public String getClient(HttpServletRequest request,ModelMap model,
        PartySite partySite) {
        locationMethod(request, model, partySite);
        return "clientLocation.tilesView";
    }

	private void locationMethod(HttpServletRequest request, ModelMap model,
			PartySite partySite) {
    ...
    @RequestMapping(value = "/mobileclient/mobileclientLocation.do", method = RequestMethod.GET)
    public String getMobileClient(HttpServletRequest request,ModelMap model,
        PartySite partySite) {
    	locationMethod(request, model, partySite);
