        ...
        var validator = $("#partyAccount").validate({
            ...
            messages: {
                'accountEntityName': {
                    ...
                    alphabetsOnly: "No Special Charcters allowed. Please enter a valid Inurance Firm Name",
                    minlength: "Enter Name with at least {0} characters.",
                    maxlength: "Enter Name with no more than {0} characters."},
                'accountNumber': {
                    required: "Policy Number is required",
                    alphaNumeric:"No Special Charcters allowed. Please enter a valid Policy Number",
                    digits: "Enter Policy Number only digits.",
                    minlength: "Enter Policy Number with at least {0} characters.",
                    maxlength: "Enter Policy Number with no more than {0} characters."},
...
<c:url value="/mobileclient/mobileClientInsurance.do" var="mobileInsuranceModuleUrl">  
    <c:param name="clientId" value="${param.clientId}"/>
    </c:url>
    
  <security:authorize access="hasRole('ROLE_FIRM_PARTNER')">
	                	<a href="<c:out value='${mobileInsuranceModuleUrl}'/>" style="text-decoration:none;">
	                    	 <button type="button" id="showInsuranceForm" onclick="showInsuranceForm()"
...
<form:form name="partyAccount" modelAttribute="partyAccount" method="POST"
           action="mobileClientInsurance.do">
...
<input type="hidden" name="baseURL" id="baseURL"
       value="mobileClientInsurance.do"/>
...
<b><strong>${inValidClientId}</strong></b>
            ...
            <div data-role="fieldcontain" ><form:label path="accountEntityName">Insurance Firm Name</form:label>
            <form:input cssClass="input_full" path="accountEntityName" class="input"
                        tabindex="1"/></div>
          
           <div data-role="fieldcontain" ><form:label path="accountNumber">Policy Number</form:label>
		...
		<div data-role="fieldcontain" ><form:label path="terms" cssClass="input_full">Memo</form:label>    
            <form:textarea path="terms" rows="3" cssClass="input_full" tabindex="16"/>      </div>
            	...
            	<security:authorize access="hasAnyRole('ROLE_STAFF','ROLE_FIRM_ADMIN')">
		     	...
		     	<a href="<c:out value='${mobileInsuranceModuleUrl}'/>" style="text-decoration:none;">
	                    	 <button type="button" id="form" onclick="hideInsuranceForm()"
	                        	    class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only"
	                            	role="button" aria-disabled="false" data-inline="true" data-mini="true">
    ...
    <div id="insurancegrid">
    <div id="grid_wrapper">
        <table id="mobinslist"></table>
        <div id="mobinspager" class="scroll"></div>
    </div>
 
 <script >
	clientId = $("#clientId").val();
	 jQuery("#mobinslist").jqGrid({
		  "hoverrows":false,
          "viewrecords":true,
          ...
          "gridview":true,
          "loadonce":true,
          "url":'<c:url value="/client/accountAjaxList.do"/>?clientId='+clientId+'&accountCategory=701',              
          "scrollPaging":true,
          pager : jQuery('#pager'),
          "rowNum":10,
          "rowList" : [20,40,60],
	    ...
	    colModel :[ 
	       ...
	       {name:'accountFirmName', index:'accountFirmName',  width:200,formatter: 'showlink', formatoptions: {baseLinkUrl: 'mobileClientInsurance.do', 
		...
		jQuery("#mobinslist").jqGrid('filterToolbar', {
			stringResult : true,
			searchOnEnter : false
		});
		 $('#mobinspager').css("height", "50px");	
		 </script>
		 </div>
		 </body>
